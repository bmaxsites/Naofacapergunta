...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ──────────────────────────────────────────────────────────
     9. HERO VIDEO — fallback de poster se o vídeo falhar
  ────────────────────────────────────────────────────────── */
  const heroVideo = document.querySelector('.hero-video');
  if (heroVideo) {
    heroVideo.addEventListener('error', () => {
      heroVideo.style.display = 'none';
      // O poster já cobre o fundo via CSS background no elemento pai
    });
    // Auto-play policy: tenta reproduzir explicitamente
    heroVideo.play().catch(() => {
      // Silencia erros de política de autoplay em alguns browsers
    });
  }
